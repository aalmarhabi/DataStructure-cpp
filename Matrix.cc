...
public:
  ...
  void partialPivot(int i, vector<double>& B) {
    ...
    // swap rows biggestpos,i
     // (*this)(biggestPos,j) =(*this)(biggestPos,j) + (*this)(i,j);
     double temp;
     for (int j = 0; j < rows; j++) {
         //cout << (*this)(biggestPos, i);
//         (*this)(i, j) = (*this)(i, j) + (*this)(biggestPos, i);
//         (*this)(biggestPos, j) = (*this)(biggestPos, j) - (*this)(i, j);
//         (*this)(i, j) = (*this)(i, j) - (*this)(biggestPos, j);
        temp = (*this)(i,j);
  ...
  friend void backSubstitute(Matrix& m, vector<double>& x, vector<double>& B) {
      //x[m.rows - 1] = B[m.rows - 1] / m(m.rows - 1, m.rows - 1);
      //cout << x[m.rows -1 ]<<endl;
